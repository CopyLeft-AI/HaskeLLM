...
example_2_6_6 text mergesTXT extensions = NVec2I $ fromListUnboxed (Z :. 2 :. 4) $ concat $ [
...
example_2_6_7 text mergesTXT extensions = NVec2I $ fromListUnboxed (Z :. 2 :. 4) $ concat $ [
                                          take 4 $ drop 1 $ encodeExtensions extensions $ BPER.encode initSeqGPT2 mergesTXT gpt2pattern mempty $ BSU.fromString text
...
example_2_6_8 text mergesTXT extensions = NVec3I $ fromListUnboxed (Z :. 2 :. 8 :. 4) $ concat $ [
...
data DropoutMaps = DropoutMaps (InsOrdHashMap BSS.ByteString [[Float]])
  deriving Show

-- | Our parser for a dropout map.
instance FromJSON DropoutMaps where
  parseJSON = withObject "DropoutMaps" (\v -> pure $ findDropoutMaps $ DAKM.toList v)
    where
...
data ProjectionWeights = ProjectionWeights (InsOrdHashMap BSS.ByteString [Float])
  deriving Show

-- | Our parser for a file containing projection Weights.
instance FromJSON ProjectionWeights where
  parseJSON = withObject "ProjectionWeights" (\v -> pure $ findProjectionWeights $ DAKM.toList v)
    where
...
projectionWeightsToJSON nVec2f
  | otherwise = A.encode $ projectionWeightsFromTensor nVec2f
    where
